...
class Ppt(BaseConv):
    ...
    def ppt_to_pagelist(self, bits, pupdator: ConvProgressUpdator) -> list[Page]:
        with tempfile.TemporaryDirectory() as temp_path:
            ...
            pages = pptx_extractor.extract(Path(pptx_file_path))
            pupdator.update(80)
            return pages
...
if __name__ == "__main__":
    ...
    if 0:
        logger.info(
            ppt.to_md(
                open(
                    r"D:\project\20240514magic_doc\doc_ppt\doc\【英文-课件】MIT15_082JF10_lec10.3MB.ppt",
                    "rb",
                ).read(),
                pupdator,
            )
    ...
    if 1:
        print(
            ppt.to_md(
                open(
                    "/opt/data/magic_doc/20240605/doc/MIT15_082JF10_lec10.3MB.ppt", "rb"
                ).read(),
                pupdator,
            )
        )
